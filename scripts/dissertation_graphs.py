...
GT_PATH = SCRIPT_DIR.parent / "test_subset.json"
...
def graph_baseline_comparison(output_dir):
    ...
    bar_colors = [COLORS["paper"], "#9467bd", COLORS["round"], COLORS["trunc"]]
...
# ---------------------------------------------------------------------------
# Graph 2: haltable vs fixed-step evaluation
def graph_haltable_vs_fixed(output_dir):
    ...
    colors = [COLORS["fixed"], COLORS["haltable"]] * 3

    for i, (val, steps) in enumerate(zip(exact_match, avg_steps)):
        ax.bar(x[i], val, 0.55, color=colors[i], edgecolor="black", linewidth=0.5)
        label = f"{val:.2f}%\n({steps:.1f} steps)"
        ax.text(x[i], val + 0.8, label, ha="center", va="bottom", fontsize=8)

    ax.set_ylabel("Exact Match Accuracy (%)")
    ax.set_xticks(x)
    ax.set_xticklabels(categories, fontsize=9)
    ax.set_ylim(0, 88)
    ax.set_title("Haltable vs Fixed-Step Evaluation (Full Dataset, 422,786 puzzles)")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
# ---------------------------------------------------------------------------
# Graph 3: ACT halting step distribution
def graph_act_steps_distribution(output_dir):
    pc_haltable_path = RESULTS_DIR / "round_v3" / "results_pytorch_bitnet_with_emb_haltable_max16.json"
...
# ---------------------------------------------------------------------------
# Graph 4: puzzle-level congruence between PC and ESP32
def graph_congruence(output_dir, model_name="round"):
    ...
    if model_name == "round":
        pc_path = RESULTS_DIR / "round_v3" / "results_pytorch_bitnet_with_emb_haltable_max16.json"
        esp_dir = RESULTS_DIR / "round_v3_esp32"
        esp_pattern = "results_esp32_round_haltable_*.json"
    elif model_name == "trunc":
        pc_path = RESULTS_DIR / "trunc_v3" / "results_pytorch_bitnet_with_emb_haltable_max16.json"
        esp_dir = RESULTS_DIR / "trunc_v3_esp32"
...
# ---------------------------------------------------------------------------
# Graph 5: per-puzzle token accuracy scatter (PC vs ESP32)
...
# ---------------------------------------------------------------------------
# Graph 6: ESP32 per-puzzle inference time distribution
...
# ---------------------------------------------------------------------------
# Graph 7: round vs truncation summary across all scenarios
def graph_round_vs_trunc_summary(output_dir):
    ...
    for i, v in enumerate(trunc_vals):
        if v is not None:
            ax.bar(x[i] + width / 2, v, width,
                   color=COLORS["trunc"], edgecolor="black", linewidth=0.5,
                   label="bf16-trunc" if i == 0 else "")
            ax.text(x[i] + width / 2, v + 0.8, f"{v:.2f}%", ha="center", va="bottom", fontsize=9)
        else:
            ax.text(x[i] + width / 2, 2, "TODO", ha="center", va="bottom", fontsize=9, color="red")
...
# ---------------------------------------------------------------------------
# Graph 8: computation-accuracy trade-off (exact match vs ACT steps)
def graph_computation_savings(output_dir):
    ...
    ax1.set_title("Computation-Accuracy Trade-off (Full Dataset, 422,786 puzzles)")
